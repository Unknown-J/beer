...
local invite_channel = {
	...
	func = function(name, param)
		...
		if not minetest.get_player_by_name(player_name) then
			...
			if minetest.get_player_by_name(player_name):get_meta():get_string("beerchat:muted:" .. name) == "" then
				if enable_sounds then
			...
			if players and players ~= "" then
				...
				for target in string.gmatch(","..players..",", ",([^,]+),") do
					...
					if not minetest.get_player_by_name(target) then
						...
						if minetest.get_player_by_name(target):get_meta():get_string("beerchat:muted:" .. name) == "" then
							if target ~= name then
								-- Sending the message
