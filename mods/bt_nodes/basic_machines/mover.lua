...
local get_mover_form = function(pos, player)
	...
	local form =
		...
		"field[0.25,4.7;3,1;prefer;filter;"..prefer.."]"..
		"list[nodemeta:"..pos.x..','..pos.y..','..pos.z..";filter;3,4.4;1,1;]"..
		"list[nodemeta:"..pos.x..','..pos.y..','..pos.z..";upgrade;4,4.4;1,1;]"..
...
minetest.register_node("basic_machines:mover", {
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		if listname == "filter" then
			local meta = minetest.get_meta(pos)
			local itemname = stack:to_string()
...
local function use_keypad(pos, ttl, again) -- position, time to live ( how many times can signal travel before vanishing to prevent infinite recursion ), do we want to activate again
	...
	local x0, y0, z0 = meta:get_int("x0"), meta:get_int("y0"), meta:get_int("z0")
	x0, y0, z0 = pos.x + x0, pos.y + y0, pos.z + z0
	...
	if text ~= "" then -- TEXT MODE; set text on target
		...
		if node.name == "basic_machines:keypad" then -- special modify of target keypad text and change its target
			x0, y0, z0 = tmeta:get_int("x0"), tmeta:get_int("y0"), tmeta:get_int("z0")
			x0, y0, z0 = tpos.x + x0, tpos.y + y0, tpos.z + z0
			tpos = {x = x0, y = y0, z = z0}
...
local function check_keypad(pos, name, ttl) -- called only when manually activated via punch
	...
	if pass == "" then
		...
		meta:set_int("count", iter); use_keypad(pos, machines_TTL, 0, name) -- time to live set when punched
		return
	end
...
minetest.register_node("basic_machines:detector", {
	...
	on_rightclick = function(pos, node, player, itemstack, pointed_thing)
		...
		local r, NOT, mode, op = nil
		r = meta:get_int("r"); mode = meta:get_string("mode"); op = meta:get_string("op")
	...
	mesecons = {effector = {
		action_on = function(pos, node, ttl)
			...
			local x0, y0, z0 = meta:get_int("x0") + pos.x, meta:get_int("y0") + pos.y, meta:get_int("z0") + pos.z
			local x2, y2, z2 = meta:get_int("x2") + pos.x, meta:get_int("y2") + pos.y, meta:get_int("z2") + pos.z

			local r , NOT, mode, op = nil
			r = meta:get_int("r") or 0; NOT = meta:get_int("NOT")
			node = meta:get_string("node"); mode = meta:get_string("mode"); op = meta:get_string("op")
			...
			if mode == "node" then
				...
				if op ~= "" then
					...
					x1, y1, z1 = meta:get_int("x1") + pos.x, meta:get_int("y1") + pos.y, meta:get_int("z1") + pos.z
...
-- i.e. seeing how machines running affect server performance
	description = "Toggle clockgen/keypad repeats",
	privs = {interact = true},
...
local get_distributor_form = function(pos, player)
	...
	form=form.."button[4.25,"..(0.25+(n)*0.75)..";1,1;ADD;ADD]".."button_exit[3.,"..(0.25+(n)*0.75)..";1,1;OK;OK]".."field[0.25,"..(0.5+(n)*0.75)..";1,1;delay;delay;"..delay .. "]";
...
minetest.register_node("basic_machines:distributor", {
	...
	after_place_node = function(pos, placer)
		...
		local name = placer:get_player_name(); punchset[name] = {node = "", state = 0}
	end,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if string.sub(formname, 0, string.len(fname)) == fname then
		...
		if fields.altgui then -- alternate gui without dropdown menu which causes crash for ios tablet users
			...
			local inv1, inv2 = meta:get_string("inv1"), meta:get_string("inv2")
	...
	if string.sub(formname,0,string.len(fname)) == fname then
		...
		if fields.OK == "OK" then
			...
			if meta:get_string("text")=="@" then -- keyboard mode
				...
				use_keypad(pos, machines_TTL, 0, name)
				return
			end
	...
	if string.sub(formname,0,string.len(fname)) == fname then
		...
		if fields.OK == "OK" then
			...
			local r, NOT = tonumber(fields.r) or 1, tonumber(fields.NOT) or 0
